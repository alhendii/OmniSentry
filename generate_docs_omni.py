...
class OmniSentryDocs(FPDF):
    def header(self):
        # Set header
        if self.page_no() > 1:
            self.set_font('Arial', 'I', 8)
            self.cell(0, 10, 'OmniSentry Documentation', 0, 0, 'L')
...
def generate_documentation():
    ...
    pdf.cell(0, 20, 'OmniSentry', ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.multi_cell(0, 10, 'OmniSentry is a minimalist security dashboard designed for monitoring endpoint threats. It uses pattern matching and NLP-based analysis to detect potential security issues in log files.')
    ...
    pdf.set_font('Arial', '', 12)
    intro_text = '''OmniSentry is a cybersecurity dashboard focused on endpoint threat detection. It provides a streamlined, minimalist interface for monitoring and analyzing security threats detected in system logs.
